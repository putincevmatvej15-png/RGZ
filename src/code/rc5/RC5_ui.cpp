...
void rc5EncryptText(RC5Cipher& cipher) {
...
void rc5DecryptText(RC5Cipher& cipher) {
    cin.ignore(numeric_limits<streamsize>::max(), '\n');
    string hexInput, key;
    ...
    if (result.empty() && !cipherText.empty()) {
        cout << "Ошибка: не удалось расшифровать (неверный ключ или поврежденные данные)" << endl;
        return;
    }
...
void rc5EncryptFile(RC5Cipher& cipher) {
    cin.ignore(numeric_limits<streamsize>::max(), '\n');
...
void rc5DecryptFile(RC5Cipher& cipher) {
    cin.ignore(numeric_limits<streamsize>::max(), '\n');
    string inputFile, outputFile, key;
